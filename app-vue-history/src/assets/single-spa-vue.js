...
function mount(opts, mountedInstances, props) {
  ...
  return Promise.resolve().then(() => {
    return resolveAppOptions(opts, props).then((appOptions) => {
      const oldMountedInstances = mountedInstances?.[props.name];
      if (opts?.isKeepAlive && oldMountedInstances?.vueInstance && oldMountedInstances?.domEl?.innerHTML) {
        console.log("mount???",oldMountedInstances)
        oldMountedInstances.domEl.style.display = "block";
        return oldMountedInstances.vueInstance;
      }
...
function unmount(opts, mountedInstances, props) {
  return Promise.resolve().then(() => {
    const instance = mountedInstances[props.name];
    if (opts?.isKeepAlive && instance?.domEl) {
      console.log("unmount@@@", instance);
      instance.domEl.style.display = "none";
    } else {
      if (opts.createApp) {
